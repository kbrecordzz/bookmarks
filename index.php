...
if ($_COOKIE["bookmarks_loggedin"] !== $password)
{
	echo "<form method=\"POST\" action=\"index.php?login\"><input type=\"password\" name=\"password\" placeholder=\"password\"> <input type=\"submit\" value=\"log in\"></form>";
	if (isset($_GET["login"])) { if ($_POST["password"] == $password) { setcookie("bookmarks_loggedin", $password, time()+60*60*24*365*1000); echo "<script>location.href = \"index.php\"</script>"; } }
}
else
{
echo "<style>";
...
echo "input { width: 100px; }";
echo "div { max-width: 240px; padding-left: 10px; padding-right: 10px; padding-top: 5px; padding-bottom: 5px; margin: 10px; background-color: #E2E2E2; }";
...
if (isset($_GET["del"]))
{
	$newdata = $_GET["data"]; $newdata[1] = '!'; file_put_contents("data/data.txt", str_replace($_GET["data"], "!" . $newdata, file_get_contents("data/data.txt")));
	if (isset($_GET["category"])) echo "<script>location.href = \"index.php?category=" . $_GET["category"] . "\";</script>";
	else echo "<script>location.href = \"index.php\";</script>";
}
...
$count = count($a)-2;
$limit = 30;
if (isset($_GET["all"]) || isset($_GET["s"])) $limit = 999999;
...
echo "<center><h1><a href=\"index.php\">bookmarks</a></h1></center>";
echo "<p><i>";
...
if (isset($_GET["category"])) echo "<a href=\"?category=" . $_GET["category"] . "&random\">&random</a><br>"; else echo "<a href=\"?random\">?random</a><br>";
...
while ($ii < $limit && $i >= 0)
{
	...
	{
		if (isset($_GET["random"])) $i = rand(0, $count);
		...
		if ((!isset($_GET["category"]) || isset($_GET["category"]) && strpos($a[$i], $_GET["category"] . ";") === 0) && strpos($a[$i], "!") !== 0)
		{
			...
			echo $b[3] . "</a>";
			if (isset($_GET["category"])) echo $row["title"] . "</a> <a href=\"index.php?category=" . $_GET["category"] . "&del&data=" . $b[0] . ";" . $b[1] . ";" . $b[2] . ";" . $b[3] . "\">&#128465;</a>";
...
else if (strpos($_GET["url"], "jpg") !== false || strpos($_GET["url"], "png") !== false || strpos($_GET["url"], ".gif") !== false)
{
	$img = $_GET["url"];
}
else
...
if (strpos($fdata, str_replace(";","",$category) . ";" . str_replace(";","",$_GET["url"]) . ";" . str_replace(";","", $img) . ";" . str_replace(";","", $title_prefix . $title)) === false)
{
	file_put_contents("data/data.txt", $fdata . str_replace(";","",$category) . ";" . str_replace(";","",$_GET["url"]) . ";" . str_replace(";","", $img) . ";" . str_replace(";","", $title_prefix . $title) . "\n");
}
